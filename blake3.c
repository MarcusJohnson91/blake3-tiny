...
static void compress(uint32_t *out, const uint32_t m[static 16], const uint32_t h[static 8], uint64_t t, uint32_t b, uint32_t d) {
...
static void load(uint32_t d[static 16], const unsigned char s[static 64]) {
    uint32_t *end;
...
void blake3_out(struct blake3 *ctx, unsigned char *restrict out, size_t len) {
